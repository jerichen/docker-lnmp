...
if ! [ -z $default ]; then
	...
	openssl req -nodes -new -x509 -keyout /etc/nginx/ssl/"$default".key -out /etc/nginx/ssl/"$site".crt -subj "/C=US/ST=NY/L=NYC/O=Dis/CN=$default"

	echo "--- Inserting SSL directives into site's server file. ---"
	cp /etc/nginx/sites-available/default.conf /etc/nginx/sites-available/$default.conf
	sed -i "s/localhost/dev.$site/g" /etc/nginx/sites-available/$default.conf
	sed -i "s/\var/\www/\html/var/\www/\html/\$site/\public" /etc/nginx/sites-available/$default.conf
	sed -i "/listen [::]:80;/a \\\n   ssl_certificate /etc/nginx/ssl/$site.crt;\n    ssl_certificate_key /etc/nginx/ssl/$site.key;\n\n"  /etc/nginx/sites-available/$default.conf

	cp /etc/nginx/sites-available/default_ssl.conf /etc/nginx/sites-available/$default_ssl.conf
	sed -i "s/localhost/dev.$site/g" /etc/nginx/sites-available/$default_ssl.conf
	sed -i "s/\var/\www/\html/var/\www/\html/\$site/\public" /etc/nginx/sites-available/$default_ssl.conf
